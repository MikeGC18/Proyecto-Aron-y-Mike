    ...
    static void Main()
    {
        ...
        using (var connection = new SQLiteConnection("Data Source=" + dbFile))
        {
            ...
            using (var reader = new SQLiteCommand("SELECT driverId, forename, surname, nationality FROM drivers;", connection).ExecuteReader())
            {
                while (reader.Read())
                {
                    Console.WriteLine($"{reader["driverId"]}: {reader["forename"]} {reader["surname"]} ({reader["nationality"]})");
                }
            }
            ...
            using (var reader = new SQLiteCommand("SELECT circuitId, name, country FROM circuits;", connection).ExecuteReader())
            {
                while (reader.Read())
                {
                    Console.WriteLine($"{reader["circuitId"]}: {reader["name"]} ({reader["country"]})");
                }
            }
            ...
            using (var reader = new SQLiteCommand("SELECT resultId, driverId, circuitId, position, points, raceDate FROM results;", connection).ExecuteReader())
            {
                while (reader.Read())
                {
                    Console.WriteLine($"Result {reader["resultId"]}: Driver {reader["driverId"]}, Circuit {reader["circuitId"]}, Position {reader["position"]}, Points {reader["points"]}, Date {reader["raceDate"]}");
                }
            }
        }
